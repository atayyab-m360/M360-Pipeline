...
const SUPABASE_URL = "https://rmsqddfclqxtymmluzn.supabase.co";
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJtc3FwZGRmY2xxdHltbWx1em4iLCJyb2xlIjoiYW5vbiIsImlhdCI6MTc0OTc2MzE3NiwiZXhwIjoyMDY1MzM5MTc2fQ.sQZ39At5pEGM2oWzDqwM9T0mUtlOkIKAFIKSZkGbU_8";
...
function exportCSV(clients) {
  ...
  const rows = clients.map(c => cols.map(k => `"${(c[k] || "").toString().replace(/"/g, '""')}"`).join(","));
...
const Modal = ({ client, onClose, onSave, onDelete }) => {
  ...
  const fields = [
    ...
    { label:"Company / Org",         key:"company", placeholder:"e.g. DC Muslim Youth" },
    { label:"Contact (email/phone)", key:"contact", placeholder:"e.g. sarah@example.com" },
    { label:"Source",                key:"source",  placeholder:"e.g. Referral, Event, LinkedIn" },
  ];
    ...
    <div style={{ position:"fixed", inset:0, background:"#00000066", zIndex:100,
      ...
      <div style={{ background:"#fff", borderRadius:16, padding:28, width:"100%", maxWidth:480,
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
          <h2 style={{ margin:0, fontSize:18, fontWeight:700, color:"#111" }}>{isEdit ? "Edit Client" : "Add Client"}</h2>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Realtime subscription — updates instantly for all users
  ...
  const saveClient = async (data) => {
    if (data.id) {
      const { error } = await supabase.from("clients").update({
        ...
        source: data.source, status: data.status, assignedTo: data.assignedTo,
        notes: data.notes, lastUpdated: data.lastUpdated,
      }).eq("id", data.id);
      if (!error) showToast("✅ Client updated");
    } else {
      ...
      if (!error) showToast("✅ Client added");
    }
    setModal(null);
  };
  ...
  const quickStatus = async (id, status) => {
    await supabase.from("clients").update({ status, lastUpdated: today() }).eq("id", id);
        ...
        <div style={{ maxWidth:1100, margin:"0 auto" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
            ...
            <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
              <button onClick={() => exportCSV(clients)} style={{ padding:"8px 14px", borderRadius:8,
      ...
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"20px 16px" }}>
        ...
        {!loading && <>
            ...
            {STATUSES.map(s => {
              ...
              return (
                <div key={s} onClick={() => setFilter(f => f === s ? "All" : s)} style={{
                  ...
                  borderRadius:12, padding:"10px 14px", cursor:"pointer", minWidth:110, flexShrink:0, transition:"all .15s",
                }}>
            ...
            <div style={{ background:"#fff", borderRadius:14, border:"1px solid #e5e7eb", overflow:"hidden" }}>
              {filtered.length === 0 ? (
                <div style={{ padding:40, textAlign:"center", color:"#aaa" }}>
                  ...
                  <div style={{ fontWeight:600 }}>No clients yet</div>
                  <div style={{ fontSize:13, marginTop:4 }}>Click "+ Add Client" to get started</div>
                </div>
              ) : filtered.map((c, i) => (
                <div key={c.id} style={{
                  padding:"14px 18px",
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0, flexWrap:"wrap" }}>
                    ...
                    <button onClick={() => setModal(c)} style={{ background:"#f3f4f6", border:"none", borderRadius:6, padding:"5px 9px", cursor:"pointer", fontSize:14 }}>✏️</button>
                  </div>
                  <div style={{ color:"#ddd", fontSize:11, alignSelf:"center", flexShrink:0 }}>{c.lastUpdated}</div>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ marginTop:20, background:"#fffbeb", border:"1.5px solid #fcd34d", borderRadius:12, padding:"12px 16px",
            ...
            <div>
              <div style={{ fontWeight:700, fontSize:13, color:"#92400e" }}>Live Shared Database</div>
